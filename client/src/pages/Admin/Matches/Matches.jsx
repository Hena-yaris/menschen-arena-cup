...
import { ClipboardList, Loader2 } from "lucide-react"; // Import icons
...
const Matches = () => {
  ...
  const handleSaveScore = async (matchId, homeScore, awayScore) => {
    ...
    try {
      ...
      alert(`Score saved for match ${matchId}!`);
  ...
  return (
    <div className="text-white">
      ...
      <div className="flex items-center gap-3 border-b border-orange-500/50 pb-4 mb-8">
        ...
        <h1 className="text-3xl font-extrabold">Match Control (Scoring)</h1>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center p-10 bg-gray-800 rounded-xl border border-gray-700">
          ...
          <p className="text-gray-400 mt-2">
            Please go to "Manage Teams & Setup" to generate the round-robin
            schedule.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          {matches.map((match) => (
            <MatchRow key={match._id} match={match} onSave={handleSaveScore} />
          ))}
        </div>
      )}
    </div>
